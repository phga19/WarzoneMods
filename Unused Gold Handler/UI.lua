...
function init(root)
	...
	textColor = "Tuscany"
	mainTextColor = "Light Blue"
	buttonColor = "Lime"
	buttonColor2 = "Screamin' Green"
end
...
function numberInputField(parent, sliderMin, sliderMax, value, interactable, wholeNumbers, boxPreferredWidth, sliderPreferredWidth, prefWidth, prefHeight, flexWidth, flexHeigth)
	...
	return UI.CreateNumberInputField(parent).SetSliderMinValue(sliderMin).SetSliderMaxValue(sliderMax).SetWholeNumbers(wholeNumbers).SetValue(value).SetInteractable(interactable).SetBoxPreferredWidth(boxPreferredWidth).SetSliderPreferredWidth(sliderPreferredWidth).SetPreferredWidth(prefWidth).SetPreferredHeight(prefHeight).SetFlexibleWidth(flexWidth).SetFlexibleHeight(flexHeigth);
end
